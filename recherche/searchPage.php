...
if(isset($_POST['search'])){
    if (!empty($_POST['searchBar'])) {
        ...
        for ($i = 0; $i < count($wordList); $i++) {
            $term=$wordList[$i];
            $requete = $db->prepare('SELECT firstname, lastname FROM users WHERE firstname LIKE :term OR lastname LIKE :termm LIMIT 10'); // j'effectue ma requête SQL grâce au mot-clé LIKE
